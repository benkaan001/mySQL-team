        ...
        case "Quit":
          db.end((err) => {
            if (err) {
              ...
              console.log(
                "Thank you for visiting mySQL_team!\nHave a great day!"
              );
              process.exit();
            }
          });
        ...
        validate: (userInput) => {
          if (isNaN(userInput))
            if (userInput > 5 || userInput < 1) {
              console.log(
                "Please enter a valid numeric value between 1 and 5 for the Department ID!"
              );
              return false;
            } else {
              return true;
    ...
    .prompt([
      ...
      },
      {
        ...
        validate: (userInput) => {
          if (isNaN(userInput)) {
            console.log(
              "Please enter a valid numeric value between 1 and 5 for the Role ID!"
            );
            return false;
      ...
      },
      {
        ...
        validate: (userInput) => {
          if (isNaN(userInput)) {
            ...
            console.log(
              "Please enter a valid numeric value between 1 and 5 for the Manager ID!"
            );
            return false;
          } else {
            return true;
  ...
  inquirer
    .prompt([
      {
        ...
        name: "updateEmployee",
        message:
          "Please select the employee you would like to make changes to: ",
        choices: [
          "Captain",
          "Veda",
          "Bilkem",
    ...
    .then((response) => {
      db.query(
        ...
        [response.newRoleId, response.updateEmployee],
        (err, res) => {
          if (err) {
            console.log(err);
          } else {
...
viewEmployeesByDepartment = () => {
  inquirer
    ...
    .then((response) => {
      db.query(
        `SELECT * FROM employees, roles WHERE employees.role_id = roles.department_id = ?`,
